...
export async function processResumeRequest(input, opts = {}) {
  ...
  const subject = `New Resume Request — ${name}${org ? ` (${org})` : ''}`;
  const text =
...
// Note: the live newsletter uses the official Substack embed (iframe) in the
// page, because Substack blocks server-side subscribes behind Cloudflare. This
// endpoint remains as an optional first-party capture path (email/log) if you
// ever want to collect signups yourself and import them.
